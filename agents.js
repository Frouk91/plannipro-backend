  ...
  try {
    const result = await db.query(`
      SELECT a.id, a.first_name, a.last_name, a.email, a.role,
             a.avatar_initials, a.can_book_presence_sites, a.agent_display_order,
             a.sub_team, a.is_active, t.name as team_name
      FROM agents a
...
router.patch('/:id', async (req, res) => {
  try {
    const { first_name, last_name, email, role, team, password, can_book_presence_sites, sub_team, is_active } = req.body;
    ...
    const { rows } = await db.query(
      `UPDATE agents SET ${fields.join(', ')} WHERE id = $${idx}
       RETURNING id, first_name, last_name, email, role, avatar_initials, can_book_presence_sites, sub_team, is_active`,
      values
    );
