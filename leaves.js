...
router.post('/', [
  ...
  try {
    ...
    let total_days = await countWorkingDays(start_date, end_date);
    // Pour les demi-journées (veille de CP/férié, ½ CP, ½ RTT), accepter total_days = 0
    const isHalfDayType = leaveType.label && (
      leaveType.label.toLowerCase().includes('½') ||
      leaveType.label.toLowerCase().includes('1/2') ||
      leaveType.label.toLowerCase().startsWith('veille')
    );
    if (total_days === 0 && !isHalfDayType) {
      return res.status(400).json({ error: 'Aucun jour ouvré sur cette période.' });
    }
    if (total_days === 0 && isHalfDayType) total_days = 0.5;
